...
if (h("NN_struct/layer1/value") == "LSTM") | (h("NN_struct/layer1/value") == "Conv1D"):
    ...
    train_spreads  =spreads[:round(spreads.shape[0] * (split_point))]
    ...
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!                :round(Dataset_Y.shape[0] * (split_point)): ???
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!                _                                         _
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    train_y = Dataset_Y[:round(Dataset_Y.shape[0] * (split_point)):,:]
    test_y = Dataset_Y[round(Dataset_Y.shape[0] * (split_point)):,:]
...
if (h("NN_struct/layer1/value") == "LSTM") | (h("NN_struct/layer1/value") == "Conv1D"):
    ...
    for i in range(0,train_X.shape[0]):
        line = ''          
        line = line + (str)(math.tan((train_X[i,window_size - 1,bid_column_index] - 0.5) * math.pi)) + ';'
        line = line + (str)(math.tan((train_X[i,window_size - 1,ask_column_index] - 0.5) * math.pi)) + ';'
        line = line + (str)(train_spreads[i]) + ';'
        if (predicted[i,2] > predicted[i,1]) & (predicted[i,2] > predicted[i,0]):
            line = line + "1"
        if (predicted[i,1] > predicted[i,2]) & (predicted[i,1] > predicted[i,0]):
            line = line + "0"
