...
namespace Экспертная_система
{
    internal class AlgorithmOptimization
    {
        ...
        public AlgorithmOptimization(Algorithm algorithm, MainForm form1, int population_value, int mutation_rate, int architecture_variation_rate, double elite_ratio, int iterarions, int test_count, TargetFunctionType target_function_type)
        {
            ...
            for (int i = 0; i < population_value; i++)
                variablesVisualizer.parameters[1].functions.Add(new Function(" [" + i.ToString() + "]", ParameterVisualizer.valueToColor(0, population_value, population_value - i - 1)));
        ...
        public void iteration_of_optimization()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < population_value; i++)
                {
                    ...
                    addTargetFunctionPointToReport(population[i], population[i].getValueByName("target_function"));
                }
        ...
        void addTargetFunctionPointToReport(Hyperparameters h,string tf)
        {
            ...
            { 
                ...
                for (int i = 0; i < h.nodes.Count; i++)
                {
                    ...
                    {
                        reportHeader += h.nodes[i].getAttributeValue("name") + ';';
                    }
                }
                File.WriteAllText(report_file_name, reportHeader + "Q;" + '\n');
                reportWasInitialized = true;
            }
            string reportLine = "";
            for (int n = 0; n < h.nodes.Count; n++)
            {
                if (h.nodes[n].getAttributeValue("variable") == "numerical")
                {
                    reportLine += h.nodes[n].getValue().Replace(',', '.') + ";";
                }
            }
            reportLine += tf + ';';

            File.AppendAllText(report_file_name, reportLine + '\n');
        }
